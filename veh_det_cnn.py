...
true_images = imageproc.list_pictures('res')
list_true_images = [imageproc.img_to_array(imageproc.load_img(x)) for x in true_images]
false_images = imageproc.list_pictures('res_false')
...
if not data_augmentation:
    ...
    xx = np.array(list_true_images)
    datagen.fit(xx)
    for j in range(100):
    	images_transform = [imageproc.array_to_img(datagen.standardize(datagen.random_transform(x))) for x in xx]
    	for i in range(len(images_transform)):
    		images_transform[i].save('res/aug_' + str(j) + str(i) + '.jpg')
